...
@property (nonatomic, strong)NSMutableDictionary *protocolCls;
@end

@implementation LLRouterController

+ (instancetype)sharedInstance {
    static LLRouterController *instance = nil;
...
+ (id)objectForURL:(NSString *)URL withUserInfo:(NSDictionary *)userInfo {
    LLRouterController *router = [LLRouterController sharedInstance];
...
- (NSMutableDictionary *)addURLPattern:(NSString *)URLPattern {///定位最底层路径
...
+ (void)registerClass:(Class)module forProtocol:(Protocol *)protocol {
    if (protocol != NULL && module != Nil) {
        [[[self sharedInstance] protocolCls] setObject:module forKey:NSStringFromProtocol(protocol)];
    }
    
}

+ (Class)classForProtocol:(Protocol *)protocol {
    return [[[self sharedInstance] protocolCls] objectForKey:NSStringFromProtocol(protocol)];
}

+ (void)deregisterClassForProtocol:(Protocol *)protocol {
    [[[self sharedInstance] protocolCls] removeObjectForKey:NSStringFromProtocol(protocol)];
}

- (NSMutableDictionary *)protocolCls {
    if (!_protocolCls) {
        _protocolCls = [NSMutableDictionary dictionaryWithCapacity:0];
    }
    return _protocolCls;
}
